...
public class ConnectViaCode : MonoBehaviour {
    ...
    public async void LinkStart(string code) {
        ...
        m_WebSocketBLiveClient.OnDanmaku += dm => OnDanmaku?.Invoke(dm);
        m_WebSocketBLiveClient.OnGift += gift => OnGift?.Invoke(gift);
        m_WebSocketBLiveClient.OnGuardBuy += guard => OnGuardBuy?.Invoke(guard);
        m_WebSocketBLiveClient.OnSuperChat += superChat => OnSuperChat?.Invoke(superChat);
        m_WebSocketBLiveClient.OnEnter += enter => OnEnter?.Invoke(enter);
        m_WebSocketBLiveClient.OnLiveStart += liveStart => OnLiveStart?.Invoke(liveStart);
        m_WebSocketBLiveClient.OnLiveEnd += liveEnd => OnLiveEnd?.Invoke(liveEnd);
        m_WebSocketBLiveClient.OnLike += like => OnLike?.Invoke(like);
    ...
    private static void WebSocketBLiveClientOnGuardBuy(Guard guard) {
        StringBuilder sb =
            new StringBuilder(
                $"用户[{guard.userInfo.userName}]充值了{guard.guardNum}个月[{(guard.guardLevel == 1 ? "总督" : guard.guardLevel == 2 ? "提督" : "舰长")}]大航海");
        Logger.Log(sb.ToString());
    }
